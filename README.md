...
![image-20260128094422599](./assets/image-20260128094422599.png)
...
- 支持导出同步所有存在未授权的请求数据包`raw`同步新增`SendRequest.py`(用于重放导出的未授权请求实现漏洞批量验证)
- 添加了数据包去重,支持排序手动删除误判的不存在未授权的请求

以如下自建灯塔系统为例,鉴权为Token字段,这里我并没有把Token加入认证头判定(一时找不到未授权的示例),所以不鉴权Token字段假设存在未授权

![QQ_1769563201077](./assets/QQ_1769563201077.png)

我们导出未授权请求

![image-20260128092551278](./assets/image-20260128092551278.png)

结构如下

![image-20260128092636820](./assets/image-20260128092636820.png)

然后添加`SendRequest.py`这个python插件,注意需要有`jython`环境

![image-20260128092758784](./assets/image-20260128092758784.png)

![image-20260128092837949](./assets/image-20260128092837949.png)

加载刚刚导出的数据包文件

![image-20260128092939722](./assets/image-20260128092939722.png)

导入后,这里有请求设置(可以根据自己的需求来),我们这里就加入Token字段把Token去掉试试

![image-20260128093028693](./assets/image-20260128093028693.png)

点击发送

![image-20260128093231681](./assets/image-20260128093231681.png)

我们先前假设存在未授权请求导出的数据包发送后Token字段移除,重发发现所有数据包长度都是216,点开看是401,可以判断不存在未授权,漏洞已修复了
